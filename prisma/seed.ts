...
async function main() { const password = await bcrypt.hash(adminPassword, 12); await db.admin.upsert({ where:{email:adminEmail}, update:{password}, create:{email:adminEmail,password,name:'Quran Academy'} });
 ...
 await db.siteSettings.update({where:{id:'main'},data:{heroSubtitle:'Куранды туура окууга, жаттоого жана ижаза алууга жол ачкан билим ордосу.',heroDescription:'Сапаттуу билим, жеке көзөмөл жана бекем тарбия аркылуу окуучунун Куран менен байланышын күчөтөбүз.',aboutTitle:'Курандын нуру менен тарбияланган муун',aboutText:'Куран Академия — Куран жаттоо, тажвид жана ижаза багытында системалуу билим берген медресе.\nБиз ар бир окуучуга түшүнүктүү программа, тажрыйбалуу устаз жана жылуу чөйрө сунуштайбыз.',featuresTitle:'Илим, адеп жана ишеним бир жерде',programsTitle:'Максатыңызга ылайык түзүлгөн окуу багыттары',teachersTitle:'Билимин аманат катары жеткирген устаздар',galleryTitle:'Академиянын жашоосунан көз ирмемдер',contactTitle:'Билим жолун бүгүн баштаңыз.'}});
 const teachers = [
 ...
 const gallery = [
  {id:'gallery-0',image:'/images/home.jpeg',caption:'Академия'},
  {id:'gallery-1',image:'/images/mudur.jpeg',caption:'Устаздар'},
  {id:'gallery-2',image:'/images/ustaz2.jpeg',caption:'Сабактар'},
  {id:'gallery-3',image:'/images/hero-graduation.png',caption:'Бүтүрүүчүлөр'},
 ];
 for (const [order, item] of gallery.entries()) await db.gallery.upsert({where:{id:item.id},update:{...item,order},create:{...item,order}});
 ...
 await db.program.update({where:{id:'program-0'},data:{title:'Куран жаттоо',description:'Куранды туура окуу, жаттоо жана бекем кайталоо үчүн комплекстүү программа.',topics:['Жаттоо','Кайталоо','Тажвид']}});
 await db.program.update({where:{id:'program-1'},data:{title:'Ижаза',description:'Санад менен окууну өркүндөтүп, ижаза алууга системалуу даярдык.',topics:['Санад','Кыраат','Жеке көзөмөл']}});
 await db.program.update({where:{id:'program-2'},data:{title:'Араб тили',description:'Курандын тилин түшүнүү үчүн грамматика, сөз байлыгы жана сүйлөө көндүмдөрү.',topics:['Нахв','Сарф','Окуу','Сүйлөө']}});
 await db.program.update({where:{id:'program-3'},data:{title:'Англис тили',description:'Окуу, сүйлөө жана күнүмдүк баарлашуу үчүн практикалык тил программасы.',topics:['Grammar','Speaking','Reading','Listening']}});
 await db.program.update({where:{id:'program-4'},data:{title:'Диний сабактар',description:'Фикх, акыйда, хадис, сира жана тафсир боюнча системалуу негизги билим.',topics:['Фикх','Акыйда','Хадис','Сира','Тафсир']}}); }
