...
type Teacher = { id: string; name: string; position: string; bio?: string | null; image?: string | null };
...
const defaultSettings: SiteSettings = {
  ...
  contactTitle: 'Сизди Академияда күтөбүз', logo: '/images/logo.png', aboutImage: '',
...
export function DashboardClient({ data }: { data: Data }) {
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (tab === 'teachers') body = { name: form.get('name'), position: form.get('position'), bio: form.get('description'), image: String(form.get('image') || '') || undefined };
