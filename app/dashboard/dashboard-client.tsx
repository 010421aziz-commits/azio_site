...
type SiteSettings = { heroLocation: string; heroTitle: string; heroSubtitle: string; heroDescription: string; aboutTitle: string; aboutText: string; featuresTitle: string; programsTitle: string; teachersTitle: string; galleryTitle: string; contactTitle: string; stats: { value: string; label: string }[] };
...
export function DashboardClient({ data }: { data: Data }) {
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (type === 'settings') {
      ...
      const response = await fetch('/api/site-settings', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ heroLocation: form.get('heroLocation'), heroTitle: form.get('heroTitle'), heroSubtitle: form.get('heroSubtitle'), heroDescription: form.get('heroDescription'), aboutTitle: form.get('aboutTitle'), aboutText: form.get('aboutText'), featuresTitle: form.get('featuresTitle'), programsTitle: form.get('programsTitle'), teachersTitle: form.get('teachersTitle'), galleryTitle: form.get('galleryTitle'), contactTitle: form.get('contactTitle'), stats }) });
      if (!response.ok) { setError('Тексттерди сактоо ишке ашкан жок.'); setLoading(false); return; }
      setSettings(await response.json()); setLoading(false); return;
    }
