...
import { FormConfigManager } from '@/components/form-config-manager';
import { DailyRoutineManager } from '@/components/daily-routine-manager';
...
type SiteSettings = {
  heroLocation: string; heroTitle: string; heroSubtitle: string; heroDescription: string; heroImage: string;
...
type Tab = 'overview' | 'teachers' | 'programs' | 'gallery' | 'messages' | 'settings' | 'form' | 'routine';

const defaultSettings: SiteSettings = {
  heroLocation: 'БИШКЕК · КЫРГЫЗСТАН', heroTitle: 'Куран Академия',
  heroSubtitle: 'Куран жаттоо жана ижаза алуу медресеси',
  heroDescription: 'Куранды туура окууну, жаттоону жана ижаза алуу жолун үйрөтүүчү заманбап медресе.', heroImage: '',
...
export function DashboardClient({ data }: { data: Data }) {
  ...
  const labels: Record<Tab, string> = { overview: 'Жалпы', teachers: 'Устаздар', programs: 'Программалар', gallery: 'Галерея', messages: 'Кабарлар', settings: 'Настройки сайта', form: 'Форма', routine: 'Күн тартиби' };
    ...
    try {
      if (tab === 'settings') {
        ...
        const response = await fetch('/api/site-settings', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
          heroLocation: form.get('heroLocation'), heroTitle: form.get('heroTitle'), heroSubtitle: form.get('heroSubtitle'), heroDescription: form.get('heroDescription'), heroImage: form.get('heroImage'),
